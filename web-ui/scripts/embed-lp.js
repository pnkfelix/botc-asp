...
// "roles/tb/poisoner.lp" -> "rolesTbPoisonerLp"
...
const exports = lpFiles
  .map(({ path, fullPath }) => {
    const varName = pathToIdentifier(path);
    return `export const ${varName} = ${readAndEscape(fullPath)};`;
  })
  .join('\n');
...
const mapEntries = lpFiles
  .map(({ path }) => {
    const varName = pathToIdentifier(path);
    return `  ${JSON.stringify(path)}: ${varName}`;
  })
  .join(',\n');
